...
c 	open the file to save the output
	OPEN(UNIT=10, FILE="BHABHA.dat", STATUS="UNKNOWN")
	OPEN(UNIT=20, FILE="BHABHA_TOT.dat", STATUS="UNKNOWN")


	WRITE(*,*) "Enter the Sqrt(S) [GeV]"
	READ(*,*) S2

	 
	DO I=-80,80,1
	X=I*0.01D0
c       print the cos theta and dsigma/domega	
C	PRINT*, X, DSIGMA(S2,X)

c       save the cos theta and dsigma/domega into the BHABHA.dat	
	WRITE(10,*) X,DSIGMA(S2,X) 	
	END DO

c       close the Bhabha.dat file
	CLOSE(10)

	DO I=10,50,1
	S20=I*1.0D0
c	PRINT*, S2, SIGTOTAL(S2)
c       save the S2 and SIGTOTAL into the BHABHA-TOT.dat	
	WRITE(20,*)  S20, SIGTOTAL(S20)
	END DO
	CLOSE(20)

c	Calling the statistical analysis
	call chisquared(19,0,chi,chired,34.8d0)
	
c       run gnuplot and evince in the shell
...
c       dsigma/domega function in terms of Ecm and X (cos theta)
...
C       FINE-STRUCTURE CONSTANT SQUARED
c       valor obtido do PDG(2016)	
...
c       N is the number of points (data file),v is the number of variables in the model, chi is the chi-squared value , chired is the chi-squared reduced and si2 is energy of center of mass. 
	...
	open(UNIT=30, FILE="experimental-data.txt", STATUS="UNKNOWN")
...
c 	d2y is calculated in quadrature (sometimes we need to add the two erros in quadrature)
	x(N)=C(1,N)  
        y(N)=C(10,N)/s
...
c	write(*,*), x(N) , y(N) , d2y(N), yth(N) 
	...
	double precision function simpson(f,a,b,n)
	!Integration routine obtained from http://ww2.odu.edu/~agodunov/computing/programs/ 
	!==========================================================
	...
	subroutine probchi2(x,ndf)
	implicit none
	double precision x,cdf
	double precision r,ndf
	call chi_square_cdf ( x, ndf, cdf )
	r=(1.0-cdf)*100.0d0
	write(*,'(1x,a,4x,f6.3,2x,a)')"The P(chi^2,ndf) probability is =",
     &r, " % " 
	return
	end
